...
public class DrugsResource {
    ...
    private static final String ENTITY_NAME = "drugs";

    private final DrugsService drugsService;

    public DrugsResource(DrugsService drugsService) {
    ...
    public ResponseEntity<Drugs> createDrugs(@Valid @RequestBody Drugs drugs) throws URISyntaxException {
        ...
        if (drugs.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new drugs cannot already have an ID")).body(null);
        }
     ...
     * or with status 500 (Internal Server Error) if the drugs couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/drugs")
    @Timed
    public ResponseEntity<Drugs> updateDrugs(@Valid @RequestBody Drugs drugs) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, drugs.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Drugs> getDrugs(@PathVariable Long id) {
        ...
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(drugs));
    }
