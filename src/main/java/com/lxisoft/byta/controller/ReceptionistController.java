...
/**
 * Controller which populates the patient current data to database of clinic and
 * the CRUD operations.
 * 
 * @author RAFEEK
 * @author Karthi
...
public class ReceptionistController {
	...
	/*
	 * @RequestMapping(value = "/qrReader", method = RequestMethod.POST,
	 * consumes = "multipart/form-data") public Patient getUser(MultipartFile
	 * file) throws JAXBException, IOException, UserNameNotRepeatException {
	 * 
	 * // conveting xml multipart to io file File convFile = new
	 ...
	 * 
	 * // Converting xml file to Object JAXBContext jaxbContext =
	 ...
	 * 
	 * privateData.setAddress(patient.getHouse() + " ho, " +
	 * patient.getStreet());
	 * 
	 * privateData.setZip(patient.getZip());
	 * 
	 * data.setPrivateData(privateData); service.save(data);
	 * 
	 * fos.close(); return patient;
	 * 
	 * }
	 * 
	...
	public void savepatient(ClinicPatientData data) throws UserNameNotRepeatException {
		logger.debug("*********inside savePatient" + data);
	...
	public ClinicPatientData updatePatient(ClinicPatientData data) {
		logger.debug("********inside updatePatient**********" + data);
		...
		service.save(datas);
		logger.debug("***************return data" + datas);
		return datas;
	}

	@RequestMapping(value = "/read/{id}", method = RequestMethod.GET)
	public ClinicPatientData readPatient(@PathVariable Long id) {
		logger.debug("********inside readPatient**********" + id);
		logger.debug("***************return service.findone(id)" + service.findOne(id));
		return service.findOne(id);
	}

	/*
	 * @RequestMapping(value="/delete{id}", method = RequestMethod.DELETE)
	 * public void deletePatient(@PathVariable Long id) { service.delete(id);
	 * 
	 * }
	 * 
	...
	public Page<ClinicPatientData> serchByName(@PathVariable String name, @PathVariable int pageNumber) {
		...
		logger.debug("********inside serchByName**********");

		logger.debug(
				"********return getPatientData(name, pageNumber)**********" + service.getPatientData(name, pageNumber));
	...
	public ClinicPatientData findByPhoneNo(@PathVariable Long phoneNo) {
		logger.debug("********findByPhoneNo**********" + phoneNo);

		logger.debug("********return service.findByPhoneNo(phoneNo) **********" + service.findByPhoneNo(phoneNo));
	 ...
	 * 
	 * @RequestMapping("/generateToken/{username}") public String
	 * generateToken(@PathVariable String username ) {
	 * 
	 * SecureRandom random = new SecureRandom(); long longToken = Math.abs(
	 * random.nextLong() ); String randomtoken = Long.toString( longToken, 16 );
	 * return ( username + ":" + randomtoken ); }
	 */

	@RequestMapping("/generateToken/{name}")
	public String generateToken(@PathVariable String name) {
		logger.debug("********inside generateToken**********" + name);

		Token token = new Token(name);
		logger.debug("********inside Token**********" + token);

		service.save(token);
		logger.debug("********return token.toString(); **********" + token.toString());
	...
	public ClinicPatientData getPatientData(@PathVariable Long id) {
		logger.debug("********inside getPatientData **********" + id);
